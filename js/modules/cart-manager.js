...
class CartManager {
  constructor() {
    ...
    console.log('CartManager создан, инициализируем...');
    this.init();
  }
  ...
  init() {
    ...
    try {
      ...
      this.showError('Ошибка при инициализации корзины');
    }
  }
  ...
  getCartTotal() {
    try {
      ...
      console.error('Ошибка при подсчете общей суммы:', error);
      return 0;
    }
  }
  ...
  renderCart() {
    try {
      ...
      const cartContent = document.getElementById('cart-content');
      if (!cartContent) {
        console.error('Элемент cart-content не найден');
        return;
      }
      ...
      console.log('Корзина отрендерена успешно');
      this.initCheckoutForm();
    } catch (error) {
      console.error('Ошибка при рендеринге корзины:', error);
      this.showError('Ошибка при отображении корзины: ' + error.message);
    }
  }
